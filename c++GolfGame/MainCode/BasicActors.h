...
namespace PhysicsEngine
{
	...
	///golfball class
	class GolfBall : public DynamicActor
	{
	public:
		...
		// - dimensions: 1m
		// - denisty: 1kg/m^3
		GolfBall(const PxTransform& pose=PxTransform(PxIdentity), PxReal radius=.2f, PxReal density=0.5f) 
			: DynamicActor(pose)
		{ 
	...
	public:
		...
		// - denisty: 1kg/m^3
		Box(const PxTransform& pose=PxTransform(PxIdentity), PxVec3 dimensions=PxVec3(0.05f,0.05f,0.05f), PxReal density=1.0f) 
			: DynamicActor(pose)
		{ 
	...
	///static box class used for objects that will stay in place. 
	class StatBox : public StaticActor
	{
	public:
		//a Box with default parameters:
		// - pose in 0,0,0
		// - dimensions: 1m x 1m x 1m
		// - denisty: 1kg/m^3
		StatBox(const PxTransform& pose = PxTransform(PxIdentity), PxVec3 dimensions = PxVec3(.5f, .5f, .5f), PxReal density = 1.f)
			: StaticActor(pose)
		{
			CreateShape(PxBoxGeometry(dimensions), density);
		}
	...
	{
	public: 
		GolfClub(const PxTransform& pose = PxTransform(PxIdentity), PxReal density = 1.f, PxVec3 otherShape = PxVec3()) : DynamicActor(pose)
		{
	...
	//hole flag
	class Flag : public DynamicActor
	{
	public:
		Flag(const PxTransform& pose = PxTransform(PxIdentity), PxReal density = 1.f, PxVec3 otherShape = PxVec3()) : DynamicActor(pose)
		{
			CreateShape(PxBoxGeometry(PxVec3(.5f, .08f, .05f)), density);
			CreateShape(PxBoxGeometry(PxVec3(.05f, 2.f, .05f)), density);
			GetShape(1)->setLocalPose(PxTransform(PxVec3(.2f, 2.f, .0f)));
	...
	class Edge : public StaticActor
	{
	public:
		Edge(const PxTransform& pose = PxTransform(PxIdentity), PxVec3 dimensions = PxVec3(.5f, .5f, .5f), PxReal density = 1.f)
			: StaticActor(pose)
		{
			CreateShape(PxBoxGeometry(dimensions), density);
			//CreateShape(PxBoxGeometry(dimensions), density);
